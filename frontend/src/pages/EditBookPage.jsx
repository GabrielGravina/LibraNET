...
export default function BookPage() {
    ...
    useEffect(() => {
        const fetchBookData = async () => {
            try {
                const response = await fetch(`http://127.0.0.1:5000/api/livro/${livro_id}`);
                if (!response.ok) {
    ...
    {bookData && (
        <div className="flex-col mb-6 p-6 bg-gray-100 rounded-md shadow-md">
            <h3 className="text-xl font-semibold text-gray-700 mb-4">Informações do Livro</h3>
            <p className="text-gray-600 text-2xl mb-2"><strong>Título:</strong> {bookData.titulo}</p>
            <p className="text-gray-600 mb-2"><strong>Autor:</strong> {bookData.autor}</p>
            <p className="text-gray-600 mb-2"><strong>Ano de Publicação:</strong> {bookData.ano_publicado}</p>
            <p className="text-gray-600 mb-2"><strong>Prateleira:</strong> {bookData.prateleira}</p>
            <p className="text-gray-600 mb-2"><strong>Disponível:</strong> {bookData.disponivel ? "Sim" : "Não"}</p>
            <p className="text-gray-600 mb-2"><strong>Status:</strong> {bookData.status}</p>
            <p className="text-gray-600 mb-2"><strong>Biblioteca:</strong> {bookData.biblioteca_id}</p>
        </div>
    )}
